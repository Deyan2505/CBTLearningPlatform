...
които имплементацията е подценила** — възстановени са в секциите, които одитът им отрежда: U16 („нужда да страдат“ + ефектът на
доминото) и U3 (изкривената негативна когниция като основна характеристика на депресията + краткосрочното лечение) в 03; U19
(Бек обучава специализантите) и U11 (над 500 проучвания) в 06. Deferred/Excluded KU остават извън страницата. **(B/C) Мисловни
карти:** Седмица 1 и Седмица 2 получиха Weekly Mind Map (Preview в нова секция `00 · Карта на седмицата`, Review в 09) — един
семантичен модел, две рендирания; нито един одобрен номер на секция не е променен (конвенцията на Седмица 3).
**(D) Седмица 10 — визуален → интерактивен модел:** шестте категории от 10.2, приложени върху одобрения случай Сали/Карен,
вече се **манипулират**, не само се четат: нов споделен, data-driven engine `CaseExaminationSimulator` поставя случая в изходно
състояние и учащият го променя — избира въпрос, предсказва какво ще разкрие, потвърждава; всяко приложено намиране остава на
дъската, а финалната преоценка (90% → ~20%) се показва едва след всичките шест. Статичният списък и explorer-ът остават.
**(E) Mind Map политика (ADR-012):** Weekly Mind Map става пети gate. 12 от 15 седмици вече имаха карта → липсата е дефект.
**Единственото известно изключение е Седмица 12** — не е поправена мълчаливо, а открито не минава новия gate (вече беше
`StructuralEnrichmentRequired`). Останалите седмици не са пипани — само декларирани според наличното.
**Това НЕ е owner approval и НЕ е lock.**

Предходна стъпка — `ACTIVE LEARNING ENRICHMENT — PHASE 2, BATCH 1 (Седмици 1, 2, 10)` (2026-09-21). Структурно обогатяване само с общия Active Learning
...
Предходна стъпка — `FINAL WHOLE-COURSE QA MILESTONE` (2026-09-19). All 15/15 course weeks are `OWNER APPROVED / LOCKED`.
Next phase: comprehensive whole-course quality assurance before marking the project `PROJECT COMPLETE` (сега допълнена с Active Learning Gate):
